...
{
//#여기사람있어요
	if (IDName == L"Smith" || IDName == L"WarriorA" || IDName == L"WarriorB" || IDName == L"WarriorC")
	...
	if (m_pResourceManager->m_minmaxList.count(_newMeshInfo->NodeName + +L"Col") == 0)
		return;
...
void DXObjectFactory::MakeHierarchy_VS4(std::vector<DXObjectBase*>& meshObject)
{
	...
	for (auto nowObj : meshObject)
	{
		...
		{
			if (nowObj->m_ParentName == targetObj->m_NodeName)
			{
...
void DXObjectFactory::MakeSkinnedNodeTM_VS3(std::vector<DXObjectBase*>& meshObject)
{
	...
	for (auto _bone : _Bones)
	{
		for (auto _nowObject : meshObject)
		{
			...
			if (_bone->m_bone_name == _nowNodeName)
			{
				// 첫 nodeTM를 넣어줌. 
		...
		for (auto _nowKVP : m_pResourceManager->m_NodeInfoList)
		{
			...
			case eGeomobject:
			{
				if (_nodeInfo->m_IsSkinningObj == false)
				{
					...
					_newMesh->m_isDraw = false;

					if (_newMesh->m_pAnimation == nullptr)
					{
