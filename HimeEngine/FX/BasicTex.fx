...
float4 RimLight_PS(VertexOut pin, uniform int gLightCount, uniform bool gUseTexure) : SV_Target
{
	// Interpolating normal can unnormalize it, so normalize it.
	pin.NormalW = normalize(pin.NormalW);
	...
	//
	// lim Light
	float Rim = 1 - max(0, dot(pin.NormalW, toEye)); // 카메라 벡터와 오브젝트의 법선벡터를 내적해서 나오는 값 판별
	 Rim = pow(Rim, 5.0f); // 강도 조정
	
	// 색 입히기 
	float3 rimColor = float3(1.f, 0.870588235f, 0.f);
	 rimColor = Rim * rimColor;
	
	float4 Final_Rim = float4(rimColor, 1);
...
technique11 Light1TexRim
{
	pass P0
	{
		SetVertexShader(CompileShader(vs_5_0, VS()));
		SetGeometryShader(NULL);
		SetPixelShader(CompileShader(ps_5_0, RimLight_PS(1, true)));
	}
}

technique11 Light2TexRim
{
	pass P0
	{
		SetVertexShader(CompileShader(vs_5_0, VS()));
		SetGeometryShader(NULL);
		SetPixelShader(CompileShader(ps_5_0, RimLight_PS(2, true)));
	}
}