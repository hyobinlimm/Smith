...
public:
	...
	bool Convert_Optimize(GeomObject* pMesh);		// 버텍스(노말), 텍스쳐 값으로 최적화를 해 준다.
	bool ConvertAll(GeomObject* pMesh);			// 모두 그냥 변환 (예시용)
	bool Convert_NewOptimize(GeomObject* pMesh);	/// 구조를 예쁘게 만든 어떤 버전
...
private:
	...
	void ApplyIndicesFromFaces(GeomObject* pMesh);	// face정보 안의 버텍스의 인덱스를 기반으로 인덱스 버퍼를 갱신해준다.
...
public:
	...
	GeomObject* m_OneMesh;						// 메시 하나의 데이터가 들어갈 구조체
	vector<GeomObject*> m_MeshList;				// 메시들
	...
	vector<Animation*> m_list_animation;		// 그 데이터의 리스트
...
private:
	...
	Vector3 Parsing_NumberRGBVector3();	// 3개의 Float를 벡터 하나로 읽어서 리턴해준다.
